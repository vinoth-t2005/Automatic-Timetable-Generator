...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'remove') {
        ...
        if ($is_lab) {
            ...
            $query = "INSERT INTO course_assignments (course_id, staff_id, periods, is_lab, lab_day, lab_periods) 
                      VALUES (:course_id, :staff_id, :periods, :is_lab, :lab_day, :lab_periods)";
            $stmt = $conn->prepare($query);
            $stmt->execute([
                'course_id' => $course_id,
                'staff_id' => $staff_id,
            ...
            // Insert into database
            $query = "INSERT INTO course_assignments (course_id, staff_id, periods, is_lab) 
                      VALUES (:course_id, :staff_id, :periods, :is_lab)";
            $stmt = $conn->prepare($query);
            $stmt->execute([
                'course_id' => $course_id, 
                'staff_id' => $staff_id,
